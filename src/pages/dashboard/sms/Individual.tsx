...
export default function Individual() {
  ...
  const { data: clients = [] } = useQuery({
    queryKey: ["sms_clients", clientTab, clientSearch],
    queryFn: async () => {
      let query = supabase.from("clients").select("id, client_id, name, contact, status, monthly_bill").order("name");
      ...
      if (clientSearch) query = query.or(`name.ilike.%${clientSearch}%,contact.ilike.%${clientSearch}%,client_id.ilike.%${clientSearch}%`);
  ...
  const sendMutation = useMutation({
    mutationFn: async () => {
      const recipients = selectedClients.length > 0
        ? clients.filter((c) => selectedClients.includes(c.id)).map((c) => c.contact).filter(Boolean)
        : number ? [number] : [];
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold">ইন্ডিভিজুয়াল এসএমএস</h1>
