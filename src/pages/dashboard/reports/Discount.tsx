...
export default function Discount() {
  ...
  const { data: rows = [], isLoading } = useQuery({
    queryKey: ["rpt-discount", a],
    queryFn: async () => {
      let q = supabase
        .from("bill_collections")
        .select("id, amount, discount, created_at, billing:billing_id(amount), client:client_id(client_id, user_id, name, zone:zone_id(name), pkg:package_id(name))")
        .gt("discount", 0)
