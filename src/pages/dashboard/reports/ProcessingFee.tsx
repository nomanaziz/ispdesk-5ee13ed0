  ...
  const { data: rows = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      let q = supabase
        ...
        .select("id, amount, payment_method, transaction_id, created_at, created_by, client:client_id(name, branch_id)")
      ...
      return (data || []).map((c: any) => {
        const branchId = c.client?.branch_id;
        const rConf = cfg.resellers.find((r) => r.branch_id === branchId);
        ...
        return {
          ...
          p_utype: branchId ? "Reseller" : "Admin",
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3 items-end">
          <div><Label className="text-xs">User Type</Label>
            <Select value={f.user_type} onValueChange={(v) => setF({ ...f, user_type: v })}>
              ...
              <SelectContent><SelectItem value="all">All</SelectItem><SelectItem value="admin">Admin</SelectItem><SelectItem value="reseller">Reseller</SelectItem></SelectContent>
            </Select>
          </div>
