...
const empty: Partial<LandingItem> = { section: "about", content_key: "", content_value: {}, sort_order: 0, is_active: true };
...
export default function WebsiteAbout() {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["landing_content", "about"],
    queryFn: async () => {
      const { data, error } = await supabase.from("landing_content").select("*").eq("section", "about").order("sort_order");
  ...
  const save = useMutation({
    mutationFn: async (item: Partial<LandingItem>) => {
      ...
      const payload = { section: "about", content_key: item.content_key, content_value: parsed, sort_order: item.sort_order, is_active: item.is_active };
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["landing_content", "about"] }); setOpen(false); toast({ title: editId ? "আপডেট হয়েছে" : "যোগ হয়েছে" }); },
  ...
  const del = useMutation({
    ...
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["landing_content", "about"] }); toast({ title: "ডিলিট হয়েছে" }); },
  });
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h1 className="text-2xl font-bold text-foreground">আমাদের সম্পর্কে পেজ</h1><p className="text-muted-foreground">About পেজের কন্টেন্ট ম্যানেজ করুন</p></div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl">
          ...
          <div className="space-y-4">
            <Input placeholder="কন্টেন্ট কী (যেমন: mission, vision)" value={form.content_key || ""} onChange={(e) => setForm({ ...form, content_key: e.target.value })} />
            <div>
