...
export default function PopTelegramSetup() {
  const { popId, branchId } = usePopScope();
  ...
  useQuery({
    ...
    queryFn: async () => {
      ...
      setUsername(data?.telegram_bot_username || "");
      setActive(!!data?.telegram_bot_active);
      return data;
    },
  });

  const { data: linked = [] } = useQuery({
    queryKey: ["pop_linked_clients", branchId],
    enabled: !!branchId,
    queryFn: async () => {
      const { data } = await supabase
        .from("clients")
  ...
  const save = useMutation({
    ...
    onSuccess: () => {
      ...
      qc.invalidateQueries({ queryKey: ["pop_telegram_bot"] });
    },
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">আপনার নিজস্ব Telegram বট দিয়ে ফ্রি বিল ও আপডেট পাঠান</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex items-center justify-between rounded-md border p-3">
              <div>
                <Label>সক্রিয়</Label>
                <p className="text-xs text-muted-foreground">নিষ্ক্রিয় থাকলে Telegram মেসেজ যাবে না</p>
              </div>
        ...
        <TelegramOptIn ownerType="pop" ownerId={popId || ""} botUsername={username} />
      </div>
