...
export default function BillReceiveDialog({ open, onOpenChange, client, billing, invalidateKey }: Props) {
  ...
  const { data: profiles = [] } = useQuery({
    queryKey: ["profiles-for-receive", isPopMode ? "pop" : "admin"],
    queryFn: async () => {
      if (isPopMode) return [];
  ...
  const availableProfiles = isPopMode
    ? [{ user_id: customer?.sub || "", full_name: customer?.name || customer?.username || "POP Admin", email: customer?.email || "" }]
    : isAdmin
      ? profiles
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isPopMode) {
        await callPortal("receive_pop_bill", {
          client_id: client.id,
          ...
          note: (isAdvance ? `Advance Pay. ${remarks}`.trim() : remarks) || null,
      ...
      <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              <Label className="text-xs">রিসিভড বাই</Label>
              <Select value={receivedBy} onValueChange={setReceivedBy}>
                ...
                <SelectContent>
                  {availableProfiles.map((p: any) => (
                    <SelectItem key={p.user_id || p.email} value={p.user_id || p.email}>
                      {p.full_name || p.email}
                    </SelectItem>
                  ))}
                </SelectContent>
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                <tr>
                  ...
                  <td className={`p-2 font-bold ${balanceDue > 0 ? "text-red-500" : "text-green-600"}`}>৳{Math.abs(balanceDue)} {isAdvance && "(অগ্রিম)"}</td>
                </tr>
              </tbody>
            </table>
          </div>
          ...
          <div className="flex items-center gap-6">
            <label className="flex items-center gap-2 text-xs"><Checkbox checked={setNextBilling} onCheckedChange={(v) => setSetNextBilling(!!v)} />পরবর্তী বিলিং তারিখ সেট করবেন?</label>
            <label className="flex items-center gap-2 text-xs"><Checkbox checked={sendSms} onCheckedChange={(v) => setSendSms(!!v)} />SMS পাঠাবেন?</label>
          </div>
        </div>

        <DialogFooter>
          <Button variant="outline" onClick={() => onOpenChange(false)}>বাতিল</Button>
          <Button onClick={handleSubmit} disabled={loading}>{loading ? "প্রসেস হচ্ছে..." : "গ্রহণ করুন"}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
