
-- 1. Create change log table
CREATE TABLE public.reseller_tariff_change_logs (
  id uuid PRIMARY KEY DEFAULT gen_random_uuid(),
  tariff_id uuid,
  tariff_package_id uuid,
  tariff_name text,
  tariff_type text,
  assigned_pops text,
  package_name text,
  server_name text,
  profile text,
  profile_speed text,
  package_rate numeric,
  validity_days integer,
  min_activation_days integer,
  effective_from date,
  effective_to date,
  changed_fields jsonb DEFAULT '[]'::jsonb,
  change_reason text,
  action text NOT NULL DEFAULT 'update',
  changed_by uuid,
  changed_at timestamptz NOT NULL DEFAULT now()
);

CREATE INDEX idx_rtcl_tariff_id ON public.reseller_tariff_change_logs(tariff_id);
CREATE INDEX idx_rtcl_changed_at ON public.reseller_tariff_change_logs(changed_at DESC);

ALTER TABLE public.reseller_tariff_change_logs ENABLE ROW LEVEL SECURITY;

CREATE POLICY "Authenticated can view tariff change logs"
  ON public.reseller_tariff_change_logs FOR SELECT
  TO authenticated USING (true);

CREATE POLICY "Authenticated can insert tariff change logs"
  ON public.reseller_tariff_change_logs FOR INSERT
  TO authenticated WITH CHECK (true);

CREATE POLICY "Super admin can delete tariff change logs"
  ON public.reseller_tariff_change_logs FOR DELETE
  TO authenticated USING (public.has_role(auth.uid(), 'super_admin'));

-- 2. Trigger function for tariff_packages changes
CREATE OR REPLACE FUNCTION public.log_tariff_package_change()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $$
DECLARE
  v_tariff record;
  v_package_name text;
  v_server_name text;
  v_pops text;
  v_changed jsonb := '[]'::jsonb;
BEGIN
  -- Lookup names
  SELECT name, type INTO v_tariff FROM public.reseller_tariffs WHERE id = NEW.tariff_id;
  SELECT name INTO v_package_name FROM public.isp_packages WHERE id = NEW.package_id;
  IF NEW.mikrotik_server_id IS NOT NULL THEN
    SELECT name INTO v_server_name FROM public.mikrotik_devices WHERE id = NEW.mikrotik_server_id;
  END IF;
  SELECT string_agg(name, ', ') INTO v_pops
    FROM public.branch_managers WHERE tariff_id = NEW.tariff_id;

  IF TG_OP = 'UPDATE' THEN
    IF COALESCE(OLD.package_id::text,'') IS DISTINCT FROM COALESCE(NEW.package_id::text,'') THEN
      v_changed := v_changed || '"package_name"'::jsonb;
    END IF;
    IF COALESCE(OLD.mikrotik_server_id::text,'') IS DISTINCT FROM COALESCE(NEW.mikrotik_server_id::text,'') THEN
      v_changed := v_changed || '"server_name"'::jsonb;
    END IF;
    IF COALESCE(OLD.mikrotik_profile,'') IS DISTINCT FROM COALESCE(NEW.mikrotik_profile,'') THEN
      v_changed := v_changed || '"profile"'::jsonb;
    END IF;
    IF COALESCE(OLD.profile_speed,'') IS DISTINCT FROM COALESCE(NEW.profile_speed,'') THEN
      v_changed := v_changed || '"profile_speed"'::jsonb;
    END IF;
    IF COALESCE(OLD.package_rate,0) IS DISTINCT FROM COALESCE(NEW.package_rate,0) THEN
      v_changed := v_changed || '"package_rate"'::jsonb;
    END IF;
    IF COALESCE(OLD.validity_days,0) IS DISTINCT FROM COALESCE(NEW.validity_days,0) THEN
      v_changed := v_changed || '"validity_days"'::jsonb;
    END IF;
    IF COALESCE(OLD.min_activation_days,0) IS DISTINCT FROM COALESCE(NEW.min_activation_days,0) THEN
      v_changed := v_changed || '"min_activation_days"'::jsonb;
    END IF;
    IF COALESCE(OLD.effective_from::text,'') IS DISTINCT FROM COALESCE(NEW.effective_from::text,'') THEN
      v_changed := v_changed || '"effective_from"'::jsonb;
    END IF;
    IF COALESCE(OLD.effective_to::text,'') IS DISTINCT FROM COALESCE(NEW.effective_to::text,'') THEN
      v_changed := v_changed || '"effective_to"'::jsonb;
    END IF;
    -- Skip log if nothing meaningful changed
    IF jsonb_array_length(v_changed) = 0 THEN
      RETURN NEW;
    END IF;
  END IF;

  INSERT INTO public.reseller_tariff_change_logs(
    tariff_id, tariff_package_id, tariff_name, tariff_type, assigned_pops,
    package_name, server_name, profile, profile_speed,
    package_rate, validity_days, min_activation_days,
    effective_from, effective_to, changed_fields, action, changed_by
  ) VALUES (
    NEW.tariff_id, NEW.id, v_tariff.name, v_tariff.type, v_pops,
    v_package_name, v_server_name, NEW.mikrotik_profile, NEW.profile_speed,
    NEW.package_rate, NEW.validity_days, NEW.min_activation_days,
    NEW.effective_from, NEW.effective_to, v_changed,
    CASE WHEN TG_OP = 'INSERT' THEN 'create' ELSE 'update' END,
    auth.uid()
  );

  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_log_tariff_package_change ON public.reseller_tariff_packages;
CREATE TRIGGER trg_log_tariff_package_change
  AFTER INSERT OR UPDATE ON public.reseller_tariff_packages
  FOR EACH ROW EXECUTE FUNCTION public.log_tariff_package_change();

-- 3. Trigger function for tariff name/type meta changes
CREATE OR REPLACE FUNCTION public.log_tariff_meta_change()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
AS $$
DECLARE
  v_changed jsonb := '[]'::jsonb;
  v_pops text;
BEGIN
  IF COALESCE(OLD.name,'') IS DISTINCT FROM COALESCE(NEW.name,'') THEN
    v_changed := v_changed || '"tariff_name"'::jsonb;
  END IF;
  IF COALESCE(OLD.type,'') IS DISTINCT FROM COALESCE(NEW.type,'') THEN
    v_changed := v_changed || '"tariff_type"'::jsonb;
  END IF;
  IF jsonb_array_length(v_changed) = 0 THEN
    RETURN NEW;
  END IF;

  SELECT string_agg(name, ', ') INTO v_pops
    FROM public.branch_managers WHERE tariff_id = NEW.id;

  INSERT INTO public.reseller_tariff_change_logs(
    tariff_id, tariff_name, tariff_type, assigned_pops,
    changed_fields, action, changed_by
  ) VALUES (
    NEW.id, NEW.name, NEW.type, v_pops,
    v_changed, 'update', auth.uid()
  );
  RETURN NEW;
END;
$$;

DROP TRIGGER IF EXISTS trg_log_tariff_meta_change ON public.reseller_tariffs;
CREATE TRIGGER trg_log_tariff_meta_change
  AFTER UPDATE ON public.reseller_tariffs
  FOR EACH ROW EXECUTE FUNCTION public.log_tariff_meta_change();
