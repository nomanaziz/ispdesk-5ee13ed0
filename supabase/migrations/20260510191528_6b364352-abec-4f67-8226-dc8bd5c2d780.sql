CREATE OR REPLACE FUNCTION public.pop_recharge_client_days(p_client_id uuid, p_days integer)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  SELECT id, name, mikrotik_id, mikrotik_profile, protocol
    INTO v_pkg
  ...
  v_server_id := COALESCE(v_client.mikrotik_id, v_pkg.mikrotik_id, v_pop.server_id);
  v_profile := COALESCE(v_client.profile, v_pkg.mikrotik_profile);
  v_protocol := COALESCE(v_client.protocol_type, v_pkg.protocol);

  IF v_server_id IS NOT NULL THEN
    SELECT name INTO v_server_name FROM public.mikrotik_devices WHERE id = v_server_id;
  END IF;
  ...
  FOR i IN 0..(p_days - 1) LOOP
    ...
    INSERT INTO public.pop_daily_charges (
      ...
      v_client.package_id, v_pkg.name, v_profile, v_protocol,
      v_server_id, v_server_name, v_zone.id, v_zone.name, v_subzone.id, v_subzone.name,
